...
export const rgbToRgba = (rgb: RGB, opacity = 1) => {
...
export const createColorpickers = () => {
  const container = document.querySelector(".values .colors") as HTMLDivElement;

  const createColorpicker = (color: ColorParams) => {
    ...
    element.oninput = () => {
      element.setAttribute("value", element.value); console.log(element.value);
    }
